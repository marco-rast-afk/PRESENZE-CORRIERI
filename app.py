...
if 'anagrafica_corrieri' not in st.session_state:
    st.session_state.anagrafica_corrieri = pd.DataFrame([
        ...
        {"COGNOME": "BORGIA", "FRANCESCO", "CELLULARE": "3280347303", "GIRO_FISSO": "211"},
...
# --- 1. FUNZIONI DI GENERAZIONE FILE (ECCEZIONI AL FRAGMENT) ---
...
# --- 2. APPLICAZIONE FRAMMENTI ISOLATI PER ABBATTERE IL REFRESH DELLA PAGINA ---
@st.fragment
def mostra_tabellone_presenze():
    ...
    st.info("💡 Le modifiche sono isolate: la pagina NON si sposta ad ogni inserimento. Se riassegni un furgone già occupato, verrà rimosso in background dal vecchio giro per evitare duplicati.")
    ...
    def salva_edits_giornaliero():
        if "editor_giornaliero_diretto" in st.session_state:
            ...
            # Controllo incrociato dei duplicati furgoni (in tempo reale dietro le quinte)
    ...
    st.data_editor(
        ...
        on_change=salva_edits_giornaliero
    )
